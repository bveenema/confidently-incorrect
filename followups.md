...
| A-7 | ☐ | Pre-rank sheet entered into Yahoo | Outermost fallback if the connection drops mid-draft. No API — manual entry, 60–90 min | `python -m data pre-rank --out prerank.txt` (or `--pool` a snapshot), then enter top-to-bottom in Yahoo. Export exists (D-100 / issue 13); this row closes when the sheet is in Yahoo. | |
