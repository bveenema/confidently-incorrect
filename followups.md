...
| A-2 | ☑ | Does the FantasyPros tier purchased return **stat-level** projections, not just point totals? | League scoring is heavily custom. Point totals from any source are useless here | Inspect an actual API response before committing to the subscription | **2 Sep 2026:** Live `GET /public/v2/json/nfl/2026/projections` returns full stat lines (`pass_yds`, `rush_yds`, `rec_rec`, …) plus `points*` we ignore. Not points-only. The key on disk still reports `tier=free`, `limit=10`, `public_api_limited=true` after the HOF upgrade — a new production key may need to be minted at https://secure.fantasypros.com/api-keys . HOF is required for the untruncated pool. K is `fg`/`fga`/`xpt` only. Week-1 DST has `def_pa`/`def_tyda`; week-0 DST does not. `rank_std` is on consensus-rankings, not projections. |
