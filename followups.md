...
| A-7 | ☐ | Pre-rank sheet entered into Yahoo | Outermost fallback if the connection drops mid-draft. No API — upload or paste CSV in Yahoo's Import Rankings dialog | `python -m data pre-rank --out prerank.csv` (or `--pool` a snapshot), then upload that file. Export exists (D-100 / D-101 / issue 13); this row closes when the sheet is in Yahoo. | |
...
| A-18 | ☑ | Rehearsal-mode isolation: `--state-dir` never points at the real runtime root | A mock run in the live root pollutes `kb.db` and `notes/` before the draft starts | Run `python -m draft serve --state-dir <temp>` and confirm banner + writes land there; run the explicit rejection case against the real root | **5 Sep 2026:** `draft.rehearsal.rehearsal_root` rejects a `--state-dir` that resolves to `CI_STATE_DIR` / `/srv/ci` (unit-tested). A distinct rehearsal root shows a REHEARSAL banner and writes `kb.db` there, not under the live env path. Copy tokens into the rehearsal dir; the serve root does not read the live store. |
| A-19 | ☐ | Post-draft backup copy + name leak check | Draft-night attribution is unrecoverable state. A backup without a leak scan can still block publication later | Immediately after draft, copy `kb.db` and `notes/` off the machine that ran non-rehearsal; grep backup for real manager/team names. Do not copy `identities.json` into that backup — it holds real team names (D-107). `draft-slots.json` is member ids only. If you copy the whole state dir, expect the grep to hit `identities.json`. | |
...
| A-22 | ☐ | Player pool has no bye week, so the draft packet cannot flag bye collisions on our roster | Architecture draft packet listed bye-week collisions; without a bye on `PooledPlayer` the council cannot see stacked byes | Add bye to the pool from a provider field, or accept the gap for 6 Sep and note it in the packet as absent | |
| A-23 | ☑ | Measure live council wall-clock after D-109 / D-110 | 6 Sep mock: 5 successful runs, ~7k specialist / ~11k Maddox tokens; inter-pick waits ~77–149s but `runs.ts` is start-only so duration was inferred. D-109 thinking-off on Gemini 400'd (`gm_missing`); Claude-off and compact prompts remain. | Time one our-clock run after D-110 (slate flips from fallback to council). If still >40s, change the draft GM slug off `gemini-2.5-pro` and record it. | **6 Sep 2026:** After D-110/D-111 the our-clock waits were 50s / 70s / 55s / abandoned at 180s. Bottleneck was Maddox on `gemini-2.5-pro` after the three specialists, against a 60s HTTP timeout. D-112 switches draft Maddox to Flash, 15s HTTP, 38s deadline, and always-visible tier names. |
| A-24 | ☑ | Confirm our-clock wall-clock ≤40s after D-112 | Deadline hides a late panel; if Flash still exceeds 38s the page stays on fallback and we never see a council slate | Time one our-clock run. Names must appear immediately. Council badge by 40s, or still fallback with the running indicator gone. | **6 Sep 2026 mock:** four our-clock waits 22s (Taco absent) / 25s / 31s / 27s. All finished under the 38s deadline with Flash GM (~5.5–6.4k tokens). Run 1 recorded a Taco brief then marked him absent (parse/validation, not a hang). |
