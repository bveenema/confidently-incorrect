...
| D-81 | Thin Yahoo Fantasy v2 client (OAuth 2.0 + httpx), no third-party wrapper | 401 after a single refresh attempt raises immediately (O-9); do not retry further. Tokens live on the state volume — a wrapper hides both the 401 and the path. App credentials and the token file are `$CI_STATE_DIR/tokens/yahoo-app.json` and `yahoo.json`. 403 / `additional_authorization_required` is an access-program error, not an auth failure. |
