...
| D-98 | `python -m draft serve --state-dir PATH` is rehearsal (D-92): the path is rejected when it resolves to the live runtime root (`CI_STATE_DIR` or `/srv/ci`). Every board mutation (setup, named pick, unnamed advance, undo) seeds a tier-based 5+ slate immediately, then starts a background `run_council` keyed by `packet_hash`. A newer board generation supersedes in-flight work for display (`failure_mode=superseded` if the stale worker never called the models). Pick POST never waits on OpenRouter. Model failure writes `runs.failure_mode` (D-63) and keeps the tier slate. `considered_options` are inserted with `chosen=0` before a run is treated as renderable. Tokens and `kb.db` stay under the serve root — copy `tokens/openrouter.json` into a rehearsal dir; do not read the live root. One-round rebuttal is still later work. Packet omits bye-week collisions until the pool carries byes (A-22). Optional `strategy.json` `draft` / `draft_strategy` is included when present. Council prompt pool is capped at 100 remaining players (prompt size, not a league value). | Session 3 / issue 12. Completes the pre-draft logging slice left by D-96. |
| D-99 | Every GitHub PR body, issue, comment, and issue/PR reply an agent writes has two sections in order: `## What this means` (jargon-free; readable without the repo) then `## Details` (paths, decision IDs, commands, gates). Neither may be empty. Chat replies and commit messages are out of scope. A one-line close/ship pointer that only names a PR or issue number is exempt. Recorded in `.ai/rules.md` §9 / §4. | The operator reads GitHub to decide what to do next; a jargon-only body hides that. Spec documents stay precise; GitHub is the human surface. |
| D-100 | Pre-rank export (`python -m data pre-rank`) lists complete-scoring players in `value_rank` order for Yahoo manual entry. Default length is 200 (issue 13 export size, not a league setting). `--limit` overrides. Snapshot at `$CI_STATE_DIR/player-pool.json` is preferred; `--pool PATH` selects one; `--refresh` rebuilds from live APIs and does not write a snapshot. Incomplete K/DST stay off the list (D-88 / A-13). Output is a numbered TSV on stdout or `--out`. No Yahoo API. A-7 stays open until the sheet is entered by hand. | Issue 13. Autopick fallback if the connection drops. Ordering must not follow published ADP. |
| D-101 | Pre-rank file format is CSV `rank,name,team,position` (name is the match key) for Yahoo's in-browser Import Rankings dialog. No comment header in the file. Supersedes the TSV reading of D-100. | Yahoo's dialog accepts paste or CSV upload; column order is name then team then position. |
| D-102 | Draft-night notes go through a deterministic append tool that writes `{state}/notes/draft.md` with YAML front matter (`date` in America/New_York ISO, `persona`, `season_id`, `entity_type`, `event`, `week`, optional `overall` / `player_key`). Lasso is invoked only for `draft-open` (POST `/setup`) and `draft-close` (first time the board is complete); a missing or failed Lasso call does not block pick entry. One structured observation per our player pick is appended from the recorded pick plus the on-page slate captured before persist, attributed to `maddox`, without an extra model call. Board-change recomputes never append. The pick-observation cap is 15 (packet-size guard, not a league setting). `notes/managers/` is not read into packets. Undo retracts the matching pick note and a close note if the draft is no longer complete. | Issue 49. Records the tool schema, file layout, and per-pick author that D-95 left open. Season-wide vault conventions stay issue 32. |
| D-103 | Specialist brief `persona` is stamped by the orchestrator from the invoked handle. A missing or wrong value in model JSON is overwritten, not rejected. The prompt no longer offers a menu of handles. | Rehearsal 5 Sep 2026: Brand and Taco were absent on 14 of 17 runs because they echoed another name. Maddox then mostly heard Belichuk. Issue 59. |
| D-104 | Draft council (OpenRouter panel) runs only when the next overall pick is ours. Every board mutation still seeds a tier fallback slate immediately and bumps the latest-only generation so an in-flight our-clock run cannot paint a stale board. Other clocks do not call the models or write a new run. | Rehearsal 5 Sep 2026: a 16-pick mock cost ~$1.20 because the panel ran on every pick. Other-clock runs do not start our useful pass any earlier. Supersedes the "every board mutation starts run_council" reading of D-98. |
| D-105 | Draft note appends and retracts run before persist (board save + council schedule) so an undo that returns us to the clock cannot feed a retracted pick note into the next packet. Lasso `draft-close` is generation-gated: complete and undo each bump a notes generation; a worker whose generation is stale does not append. Close remains idempotent on `event`. | Critique of D-102 + D-104: persist-then-retract left the undone pick in the undo packet; an in-flight close after undo is idempotent on event and would block the real close. |
| D-106 | Draft page is glanceable and live: `GET /state` (JSON, generation heartbeat) is polled in-page; the council panel is captured on the worker into `DraftSlate` and recovered from `kb.db` once at startup by `packet_hash`; `$CI_STATE_DIR/draft-slots.json` is the D-17 mapping (display names on the localhost page only, `?names=pseudonym` for screenshots). The mapping is never in a council packet or `draft-board.json`. Our-pick notes may name seats as `slot-N` codes. No note is added on other teams' picks (D-95 / D-102 stand). | Issues 10 and 50. Closes the unstyled-form reading of D-80 / D-89 for draft night without FastAPI or Yahoo polling. |
| D-107 | League identity and draft order are two files. `$CI_STATE_DIR/identities.json` is the D-17 table (stable `mN` id, `display` real team name, `pseudonym` e.g. `manager-1`). `$CI_STATE_DIR/draft-slots.json` is slot → member id only — no real names. Notes and packets never read `display`. Our-pick notes use the member `pseudonym`, not `slot-N`. Unseating a slot keeps the member for later reuse. Supersedes the "draft-slots.json is the D-17 mapping" and `slot-N` note-code readings of D-106. | A slot-keyed identity cannot be reused after the draft; next year's order would rename the person. Issue 18 will consume the same identities file at Yahoo ingest. |
| D-108 | Our draft seat is not typed. `$CI_STATE_DIR/identities.json` marks our team (`ours: true`, or a `display` matching the public name Confidently Incorrect). The localhost page is a reorderable list: dragging the room into snake order writes `$CI_STATE_DIR/draft-slots.json`; whichever row holds our member becomes `our_slot` in `draft-board.json`. Reorder is refused after the first pick. The public team name is the only hardcoded identity string, used only to find our row when the flag is missing, and is never sent in a packet. Supersedes the "Our slot is entered on the page" reading of D-89. | Draft order is unknown until the room opens. Typing a slot number when the row is already on the page is a footgun. |
| D-109 | Draft slate is need-aware. Open starting slots come from `$CI_STATE_DIR/league-settings.json` `roster_slots` versus our roster — never a hardcoded "one QB". The council pool, the displayed GM slate, and the tier fallback all skip a position whose remaining starter need is 0 (flex still accepts WR/RB/TE). QB/K/DST are capped at their dedicated slot count even in bench rounds. OpenRouter completions for Gemini and Claude send `reasoning.enabled=false` so thinking tokens do not eat the ~60s clock. Packet JSON in the prompt is compact. The localhost slate rows copy the player name to the clipboard. | 6 Sep 2026 mock: Brand/Taco stacked QBs despite `positional_scarcity` in the packet; wall time was max(specialists)+Maddox on `gemini-2.5-pro` (~7k/11k tokens) and often exceeded 40s, once past 60s. The packet already listed need; models ignored it, so the filter is deterministic. |
