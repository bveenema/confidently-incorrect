...
| A-9 | ☐ | Verify runtime state at `/srv/ci/` is outside the git working directory | A checkout, stash, or clean would destroy the season's attribution data. Sharpest footgun in the deploy story | Run `git clean -nxd` on the VPS and confirm nothing under `/srv/ci/` is listed | |
